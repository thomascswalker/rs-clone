	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float dotResult72 = dot( V , -( -_DirectionalLightDatas[0].forward + ( normalWS * _SSSFresnel ) ) );
				float4 color127 = IsGammaSpace() ? float4(0,0,0,0) : float4(0,0,0,0);
				float4 lerpResult125 = lerp( color127 , packedInput.ase_color , _AmbientIntensity);
				
				surfaceDescription.Albedo = packedInput.ase_color.rgb;
				surfaceDescription.Normal = float3( 0, 0, 1 );
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = (( _Metallic )?( 1.0 ):( 0.0 ));

				#ifdef _MATERIAL_FEATURE_SPECULAR_COLOR
				surfaceDescription.Specular = 0;
				#endif

				surfaceDescription.Emission = ( ( packedInput.ase_color * ( saturate( dotResult72 ) * _SSSIntensity ) ) + lerpResult125 ).rgb;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float dotResult72 = dot( ase_worldViewDir , -( -_DirectionalLightDatas[0].forward + ( ase_worldNormal * _SSSFresnel ) ) );
				float4 color127 = IsGammaSpace() ? float4(0,0,0,0) : float4(0,0,0,0);
				float4 lerpResult125 = lerp( color127 , packedInput.ase_color , _AmbientIntensity);
...
/*ASEBEGIN
Version=18921
345;231;1568;955;944.2244;938.5969;1.872216;True;True
...
WireConnection;129;4;97;0
WireConnection;129;6;124;0
WireConnection;129;7;86;0
ASEEND*/
//CHKSM=3F86781A654F0CE5B2D0442853D4C2EDEB7A3376